...
const COPY = {
  ja: {
    ...
    chips: 'チップ', chipsForPlayer: 'Player {player}のチップ', chipCount: '{count} / 3種類', chipSearch: 'チップを検索', chipSearchPlaceholder: 'チップ名・効果', chipFull: 'Player {player}のチップは3種類まで選べます。', chipSelected: 'チップを選択しました。', chipRemoved: 'チップを外しました。', chipRemove: '{name}を外す', noChips: '一致するチップがありません。', showLevels: 'タタLvを表示', flex: '自由枠', flexDescription: 'この枠は好きなタタに置き換えてOKです。', flexAdd: '＋ 自由枠', flexSelected: '自由枠を選択中。配置するマスを押してください。', changeToFlex: '自由枠に変更', flexCount: 'タタ{tata}体＋自由枠{flex}／{limit}枠'
  },
  en: {
    ...
    chips: 'Chips', chipsForPlayer: 'Player {player} chips', chipCount: '{count} / 3 types', chipSearch: 'Search chips', chipSearchPlaceholder: 'Chip name or effect', chipFull: 'Player {player} can select up to 3 chip types.', chipSelected: 'Chip selected.', chipRemoved: 'Chip removed.', chipRemove: 'Remove {name}', noChips: 'No chips match your search.', showLevels: 'Show Tata levels', flex: 'Flex slot', flexDescription: 'Replace this slot with any Tata you like.', flexAdd: '+ Flex slot', flexSelected: 'Flex slot selected. Tap a cell to place it.', changeToFlex: 'Change to flex slot', flexCount: '{tata} Tata + {flex} flex / {limit} slots'
  },
  'zh-CN': {
    ...
    chips: '芯片', chipsForPlayer: 'Player {player}的芯片', chipCount: '{count} / 3种', chipSearch: '搜索芯片', chipSearchPlaceholder: '芯片名称或效果', chipFull: 'Player {player}最多可选择3种芯片。', chipSelected: '已选择芯片。', chipRemoved: '已移除芯片。', chipRemove: '移除{name}', noChips: '没有符合条件的芯片。', showLevels: '显示塔塔等级', flex: '自由位置', flexDescription: '这个位置可以替换成任意塔塔。', flexAdd: '＋ 自由位置', flexSelected: '已选择自由位置，请点击格子放置。', changeToFlex: '改为自由位置', flexCount: '{tata}只塔塔＋{flex}个自由位置／{limit}格'
  }
}[locale];
...
const stageImage = (family, stage) => stageImageFor(family, stage, imageByFamily);
const stage1Image = (family) => stageImage(family, 1);
...
function cellLabel(slot, index) {
  ...
  return slot?.kind === FREE_SLOT_KIND ? `${position} ${player}${COPY.flex}` : member ? `${position} ${player}${getFamilyDisplayLabel(member.family)} T${slot.stage}${level}` : `${position} ${COPY.empty}`;
}
...
function renderBoard() {
  ...
  board.innerHTML = team.slots.slice(0, boardSlotCount(team)).map((slot, index) => {
    ...
    if (slot?.kind === FREE_SLOT_KIND) { const removeLabel = message(team.mode === 'zombie' ? COPY.quickRemove : COPY.quickRemoveBasic, { player: slot.playerId, name: COPY.flex }); const playerBadge = team.mode === 'zombie' ? `<span class="formation-player-badge">P${slot.playerId}</span>` : ''; return `<div class="formation-cell is-filled is-flex-slot is-player-${slot.playerId}${selectedMove}" data-drop-cell="${index}"><button class="formation-cell-main" type="button" draggable="true" data-cell="${index}" data-drag-cell="${index}" aria-label="${esc(cellLabel(slot,index))}"><span class="formation-flex-symbol" aria-hidden="true">?</span><b>${esc(COPY.flex)}</b>${playerBadge}</button><button class="formation-quick-remove" type="button" data-quick-remove="${index}" aria-label="${esc(removeLabel)}"><span aria-hidden="true">−</span></button></div>`; }
...
function renderPlayerSettings() {
  $('#team-player-settings').innerHTML = activePlayerIds(team).map((id) => {
    ...
    return `<section class="formation-player-card is-player-${id}"><div class="formation-player-head">${identity}<strong>${esc(message(COPY.flexCount,{tata:tataCount(team,id),flex:freeSlotCount(team,id),limit:playerLimit(team,id)}))}</strong></div>${slotSetting}${levelSetting}</section>`;
  }).join('');
}
...
function renderSelection() {
  ...
  if (selected.kind === FREE_SLOT_KIND) { node.innerHTML = `<span class="formation-flex-symbol" draggable="true" data-selected-drag data-drag-kind="${FREE_SLOT_KIND}" aria-label="${esc(COPY.dragToBoard)}">?</span><div><small>${esc(COPY.selected)}</small><b>${team.mode==='zombie'?`P${currentPlayer} · `:''}${esc(COPY.flex)}</b><span>${esc(COPY.flexDescription)}</span><small class="formation-selection-drag-hint">${esc(COPY.dragToBoard)}</small></div>`; setStatus(COPY.flexSelected,false,'#team-message'); return; }
  ...
  const image = stageImage(family, selected.stage);
  ...
  node.innerHTML = `${image ? `<img src="${esc(image.src)}"${responsiveAttrs(image)} width="64" height="64" alt="${esc(getTataDisplayName(evolution))}" draggable="true" data-selected-drag data-drag-family="${esc(family.id)}" data-drag-stage="${selected.stage}" aria-label="${esc(dragLabel)}" title="${esc(dragLabel)}">` : `<span class="formation-image-placeholder">${esc(COPY.placeholder)}</span>`}<div><small>${esc(COPY.selected)}</small><b>${player}${esc(getFamilyDisplayLabel(family))} · T${selected.stage}${level}</b><span>${esc(getTataDisplayName(evolution))}</span><small class="formation-selection-drag-hint">${esc(COPY.dragToBoard)}</small></div>`;
...
function renderModeDetails() {
  ...
  if (!node) { node = document.createElement('div'); node.id = 'team-mode-details'; node.className = 'formation-mode-details'; const advanced = document.querySelector('.astra-settings-details'); if (advanced) advanced.append(node); else $('#team-mode').closest('.tool-toolbar').after(node); }
...
function editControls(slot, member) {
  ...
  return `<div class="formation-edit-summary">${image ? `<img src="${esc(image.src)}" width="80" height="80" alt="${esc(getTataDisplayName(member.evolution))}">` : `<span class="formation-image-placeholder">${esc(COPY.placeholder)}</span>`}<div><b>${esc(getFamilyDisplayLabel(member.family))}</b><span>${esc(getTataDisplayName(member.evolution))}</span></div></div>${playerControls}<fieldset><legend>Tier</legend><div class="formation-stage-options">${member.family.evolutions.map((item) => `<button type="button" class="${Number(item.stage) === slot.stage ? 'is-selected' : ''}" data-edit-stage="${item.stage}" aria-pressed="${Number(item.stage) === slot.stage}"><span>T${item.stage}</span>${esc(getTataDisplayName(item))}</button>`).join('')}</div></fieldset>${levels}<div class="tool-actions">${copyAction}<button type="button" class="ghost-button" data-edit-change>${esc(COPY.change)}</button><button type="button" class="ghost-button" data-edit-flex>${esc(COPY.changeToFlex)}</button><button type="button" class="ghost-button" data-edit-move>${esc(COPY.move)}</button><button type="button" class="ghost-button danger-button" data-edit-remove>${esc(COPY.remove)}</button><a class="ghost-button" href="${localePrefix}/tata/${encodeURIComponent(member.family.id)}/">${esc(COPY.detail)}</a></div>`;
}
function flexEditControls(slot) { const otherPlayer=slot.playerId===1?2:1; const playerControls=team.mode==='zombie'?`<fieldset><legend>${esc(COPY.player)}</legend><div class="formation-segmented">${PLAYER_IDS.map((id)=>`<button type="button" class="is-player-${id}" data-edit-player="${id}" aria-pressed="${slot.playerId===id}">P${id}</button>`).join('')}</div></fieldset>`:''; const copyAction=team.mode==='zombie'?`<button type="button" class="ghost-button" data-copy-player="${otherPlayer}">${esc(message(COPY.copyToPlayer,{player:otherPlayer}))}</button>`:''; return `<div class="formation-edit-summary is-flex"><span class="formation-flex-symbol">?</span><div><b>${esc(COPY.flex)}</b><span>${esc(COPY.flexDescription)}</span></div></div>${playerControls}<div class="tool-actions">${copyAction}<button type="button" class="button" data-edit-change>${esc(COPY.change)}</button><button type="button" class="ghost-button" data-edit-move>${esc(COPY.move)}</button><button type="button" class="ghost-button danger-button" data-edit-remove>${esc(COPY.remove)}</button></div>`; }
function openEditor(index) { const slot = team.slots[index]; if (!slot) return; const member = memberFor(slot); if (!member && slot.kind!==FREE_SLOT_KIND) return; editingIndex = index; $('#team-edit-content').innerHTML = slot.kind===FREE_SLOT_KIND?flexEditControls(slot):editControls(slot, member); $('#team-edit-dialog').showModal(); }
function refreshEditor() { if (editingIndex === null || !team.slots[editingIndex]) return; const slot=team.slots[editingIndex]; $('#team-edit-content').innerHTML = slot.kind===FREE_SLOT_KIND?flexEditControls(slot):editControls(slot,memberFor(slot)); }
...
function setupPhase4Controls() {
  ...
  if (!localStorage.getItem(ONBOARDING_KEY) && !document.body.dataset.astra) showOnboarding();
}
...
async function exportImage() {
  ...
  activePlayerIds(team).forEach((id, index) => { const x = 62 + index * 570; context.fillStyle = id === 1 ? '#ef5f61' : '#4a91e8'; context.fillRect(x, 88 + detailOffset, 20, 20); context.fillStyle = '#f8fafc'; context.font = '700 21px sans-serif'; const settings = team.playerSettings[id]; const total=playerCount(team, id); const limit=playerLimit(team, id); const prefix = team.mode === 'zombie' ? `P${id}  ` : ''; context.fillText(`${prefix}${message(COPY.flexCount,{tata:total-freeSlotCount(team,id),flex:freeSlotCount(team,id),limit})}${team.mode === 'zombie' && settings.slotLimitPlusOne ? '  Slot+1' : ''}`, x + 32, 107 + detailOffset); });
  if (team.mode === 'zombie') {
  ...
  for (let index = 0; index < slotCount; index += 1) {
    ...
    context.fillStyle = '#171a25'; context.fillRect(x, y, cell, cell); context.strokeStyle = slot ? (slot.playerId === 1 ? '#ef5f61' : '#4a91e8') : '#30364b'; context.lineWidth = slot ? 6 : 3; if(slot?.kind===FREE_SLOT_KIND) context.setLineDash([14,9]); context.strokeRect(x, y, cell, cell); context.setLineDash([]); if(slot?.kind===FREE_SLOT_KIND){context.fillStyle='#dbeafe';context.font='800 52px sans-serif';context.textAlign='center';context.fillText('?',x+cell/2,y+cell/2-5);context.font='800 23px sans-serif';context.fillText(COPY.flex,x+cell/2,y+cell/2+35);if(team.mode==='zombie'){context.fillStyle=slot.playerId===1?'#ef5f61':'#4a91e8';context.fillRect(x+5,y+5,43,34);context.fillStyle='#fff';context.fillText(`P${slot.playerId}`,x+26,y+29);}continue;} if (!member) continue;
...
function createDragGhost(payload, native = false) {
  ...
  if (payload.kind === FREE_SLOT_KIND || (payload.type==='board' && team.slots[payload.index]?.kind===FREE_SLOT_KIND)) { const symbol=document.createElement('span'); symbol.className='formation-flex-symbol'; symbol.textContent='?'; const copy=document.createElement('span'); const name=document.createElement('strong'); name.textContent=COPY.flex; copy.append(name); ghost.append(symbol,copy); }
  else if (payload.type === 'picker') {
    const family = familyById(payload.familyId); const image = family && stageImage(family, payload.stage);
...
function applyDrop(index, payload) {
  ...
  if (team.slots[index] && team.slots[index].kind !== FREE_SLOT_KIND) { setStatus(COPY.dropOccupied, true, '#team-message'); clearDragState(); return; }
  const member = payload.kind===FREE_SLOT_KIND ? freeSlot(payload.playerId) : { familyId: payload.familyId, stage: payload.stage, playerId: payload.playerId, level: payload.level }; const issue = placementIssue(team, index, member, families);
...
function placeRandomly(pick) {
  const member = pick.kind===FREE_SLOT_KIND ? freeSlot(currentPlayer) : { ...pick, playerId: currentPlayer, level: currentLevel };
...
function bind() {
  ...
  $('#team-selection').addEventListener('pointerdown', (event) => {
    ...
    beginPointerDrag(event, source.dataset.dragKind===FREE_SLOT_KIND ? {type:'picker',kind:FREE_SLOT_KIND,playerId:currentPlayer}:{ type: 'picker', familyId: source.dataset.dragFamily, stage: Number(source.dataset.dragStage), playerId: currentPlayer, level: currentLevel }, source, { preventDefault: true, activateImmediately: true });
  });
  $('#team-selection').addEventListener('dragstart', (event) => {
    const source = event.target.closest('[data-selected-drag]'); if (!source) return;
    pointerDrag = null; dragPayload = source.dataset.dragKind===FREE_SLOT_KIND ? {type:'picker',kind:FREE_SLOT_KIND,playerId:currentPlayer}:{ type: 'picker', familyId: source.dataset.dragFamily, stage: Number(source.dataset.dragStage), playerId: currentPlayer, level: currentLevel };
  ...
  $('#team-picker-list').addEventListener('click', (event) => {
    if (Date.now() < suppressClickUntil) { event.preventDefault(); return; }
    const flexButton=event.target.closest('[data-pick-flex]'); const button = event.target.closest('[data-pick-family]'); if (!button&&!flexButton) return; const pick = flexButton?freeSlot(currentPlayer):{ familyId: button.dataset.pickFamily, stage: Number(button.dataset.pickStage) };
    if (replacingIndex !== null) { const previous = team.slots[replacingIndex]; const replacement = pick.kind===FREE_SLOT_KIND?freeSlot(previous.playerId):{ ...pick, playerId: previous.playerId, level: previous.level || Math.min(currentLevel,levelLimit(team,previous.playerId)) }; const issue = placementIssue(team, replacingIndex, replacement, families); if (!issue) { commit(placeMember(team, replacingIndex, replacement, families), COPY.placed); closePickerForPlacement(); } else reportIssue(issue, replacement.playerId); replacingIndex = null; selected = pick; return; }
    placeRandomly(pick);
  });
  $('#team-picker-list').addEventListener('pointerdown', (event) => { const handle = event.target.closest('[data-drag-handle]'); const source = handle || (matchMedia('(hover: hover) and (pointer: fine)').matches ? event.target.closest('[data-pick-family],[data-pick-flex]') : null); if (source) beginPointerDrag(event, source.dataset.dragKind===FREE_SLOT_KIND?{type:'picker',kind:FREE_SLOT_KIND,playerId:currentPlayer}:{ type: 'picker', familyId: source.dataset.dragFamily, stage: Number(source.dataset.dragStage), playerId: currentPlayer, level: currentLevel }, source, { preventDefault: Boolean(handle), activateImmediately: Boolean(handle) }); });
  $('#team-picker-list').addEventListener('dragstart', (event) => { const source = event.target.closest('[data-drag-family],[data-drag-kind]'); if (!source) return; pointerDrag = null; dragPayload = source.dataset.dragKind===FREE_SLOT_KIND?{type:'picker',kind:FREE_SLOT_KIND,playerId:currentPlayer}:{ type: 'picker', familyId: source.dataset.dragFamily, stage: Number(source.dataset.dragStage), playerId: currentPlayer, level: currentLevel }; event.dataTransfer.effectAllowed = 'copy'; event.dataTransfer.setData('text/plain', 'monsaba-tata'); source.classList.add('is-dragging'); if (matchMedia('(max-width: 820px)').matches) document.querySelector('.formation-picker')?.classList.add('is-drag-compact'); const ghost = createDragGhost(dragPayload, true); event.dataTransfer.setDragImage?.(ghost, 44, 44); });
  ...
  $('#team-edit-content').addEventListener('click', (event) => {
    ...
    if (event.target.closest('[data-edit-flex]')) { commit(placeMember(team,editingIndex,freeSlot(slot.playerId),families),COPY.changed); refreshEditor(); return; }
