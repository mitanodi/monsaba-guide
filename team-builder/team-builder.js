...
function renderSelection() {
  ...
  node.innerHTML = `${image ? `<img src="${esc(image.src)}" width="64" height="64" alt="${esc(getFamilyDisplayLabel(family))}">` : `<span class="formation-image-placeholder">${esc(COPY.placeholder)}</span>`}<div><small>${esc(COPY.selected)}</small><b>P${currentPlayer} · ${esc(getFamilyDisplayLabel(family))} · T${selected.stage}${level}</b><span>${esc(getTataDisplayName(evolution))}</span></div>`;
