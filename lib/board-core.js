...
export function validateThreadBody(body) {
  ...
  const enteredTitle = normalizeText(body.title, 'タイトル', BOARD_CONFIG.maxTitleLength, { singleLine: true });
  const title = enteredTitle || (() => {
    const singleLine = content.replace(/\s+/gu, ' ');
    const characters = [...singleLine];
    return characters.length > BOARD_CONFIG.autoTitleLength
      ? `${characters.slice(0, BOARD_CONFIG.autoTitleLength).join('')}…`
      : singleLine;
  })();
  ...
  const category = normalizeText(body.category, 'カテゴリ', 30, { singleLine: true }) || 'その他';
  if (!BOARD_CATEGORIES.includes(category)) throw new BoardError('INVALID_CATEGORY', 'カテゴリが正しくありません。');
...
export function validateAnswerBody(body) {
  ...
  const name = normalizeText(body.name, '名前', BOARD_CONFIG.maxNameLength, { singleLine: true });
  const parentAnswerId = normalizeText(body.parentAnswerId, '返信先ID', 80, { singleLine: true }) || null;
  const threadOwnerToken = normalizeText(body.threadOwnerToken, '質問者情報', 128, { singleLine: true });
  return { threadId, content, name, parentAnswerId, threadOwnerToken };
}
...
const publicAnswer = ({ deleteTokenHash, status, ...answer }) => {
  const shared = {
    ...answer,
    parentAnswerId: answer.parentAnswerId || null,
    isQuestioner: Boolean(answer.isQuestioner),
    deleted: status === 'deleted'
  };
  if (status !== 'deleted') return shared;
  return {
    id: shared.id,
    threadId: shared.threadId,
    parentAnswerId: shared.parentAnswerId,
  ...
  return {
    ...
    async getThread(id, cursor) {
      ...
      return {
        ...
        answers: result.answers.filter((answer) => ['active', 'deleted'].includes(answer?.status)).map(publicAnswer),
        nextCursor: result.hasMore ? encodeBoardCursor(offset + result.consumed) : null
      };
    },
    ...
    async createAnswer(body, ipAddress) {
      ...
      if (input.parentAnswerId) {
        const parent = await store.getAnswer(input.parentAnswerId);
        if (!parent) throw new BoardError('PARENT_ANSWER_NOT_FOUND', '返信先の投稿が見つかりません。', 400);
        if (parent.threadId !== input.threadId) throw new BoardError('PARENT_THREAD_MISMATCH', '別の質問にある投稿へは返信できません。', 400);
        if (!['active', 'deleted'].includes(parent.status)) throw new BoardError('PARENT_ANSWER_NOT_FOUND', '返信先の投稿が見つかりません。', 400);
      }
      ...
      const answer = {
        ...
        isQuestioner: safeTextEqual(sha256(input.threadOwnerToken), thread.deleteTokenHash),
    ...
    async remove({ type, id, deleteToken: suppliedToken }, suppliedAdminToken = '') {
      ...
      if (type === 'thread') await store.removeThread(normalizedId);
        ...
        const hasChildren = await store.hasAnswerChildren(record.threadId, normalizedId);
        if (hasChildren) {
          ...
          return { type, id: normalizedId, tombstone: true };
        }
        await store.removeAnswer(normalizedId, record.threadId);
      }
      return { type, id: normalizedId, tombstone: false };
    },
