...
export function createBoardRedisStore({
  ...
  return {
    ...
    async updateAnswer(answer) {
      await redis.set(answerKey(answer.id), answer);
    },
    async hasAnswerChildren(threadId, answerId) {
      const ids = await redis.zrange(answersKey(threadId), 0, -1);
      const values = (await getMany(ids.map(answerKey))).filter(Boolean);
      return values.some((answer) => answer.parentAnswerId === answerId && ['active', 'deleted'].includes(answer.status));
    },
