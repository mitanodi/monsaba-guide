...
const copy={
  ja:{month:'{y}年{m}月',heading:'イベントカレンダー',intro:'日本時間09:00を1日の境界として、開催中・次回・任意月の予定を確認できます。',prev:'前の月',next:'次の月',today:'今月',team:'チームイベント',island:'アイランドトレジャー／ゴールドラッシュ',boss:'ボスラリー',live:'開催中',nextUp:'次回',day:'{n}日目',final:'最終日',recruit:'チーム募集・編成日',recruitShort:'チーム募集',battle:'本戦 Day{n}',aggregate:'最終日・集計日',switch:'次の切り替え',unknownCycle:'4週サイクルの第1週アンカー確認待ち',unknownBoss:'2日周期の切り替えアンカー確認待ち',observed:'現在確認済み',week:['日','月','火','水','木','金','土'],note:'※すべて日本時間09:00で切り替わります。未確認の周期は推測表示しません。',error:'カレンダーを読み込めませんでした。'},
  en:{month:'{m}/{y}',heading:'Event Calendar',intro:'See what is live, what comes next, and any month at a glance. Each game day changes at 09:00 JST.',prev:'Previous',next:'Next',today:'This month',team:'Team Event',island:'Island Treasure / Gold Rush',boss:'Boss Rally',live:'Live',nextUp:'Next',day:'Day {n}',final:'Final day',recruit:'Team recruitment & setup',recruitShort:'Team recruitment',battle:'Battle Day {n}',aggregate:'Final day / tally',switch:'Next change',unknownCycle:'Week 1 anchor for the four-week cycle is not confirmed',unknownBoss:'Two-day boss change anchor is not confirmed',observed:'Observed now',week:['Sun','Mon','Tue','Wed','Thu','Fri','Sat'],note:'All changes use the 09:00 JST boundary. Unconfirmed cycles are never guessed.',error:'Could not load the calendar.'},
  'zh-CN':{month:'{y}年{m}月',heading:'活动日历',intro:'以日本时间09:00为每日分界，一目了然地查看当前、下一项及任意月份的安排。',prev:'上个月',next:'下个月',today:'本月',team:'团队活动',island:'岛屿宝藏／淘金热',boss:'Boss Rally',live:'进行中',nextUp:'下次',day:'第{n}天',final:'最后一天',recruit:'组队及编成日',recruitShort:'团队招募',battle:'正赛第{n}天',aggregate:'最后一天・结算日',switch:'下次切换',unknownCycle:'四周周期的第1周锚点尚待确认',unknownBoss:'两天Boss轮换锚点尚待确认',observed:'当前已确认',week:['日','一','二','三','四','五','六'],note:'所有切换均以日本时间09:00为准。未确认的周期不会推测显示。',error:'无法加载日历。'}
}[localeKey];
...
function currentCards(now){
  const state=calendarState(now,config);const observation=config.bossRotationConfig.currentObservation;const boss=config.bossRotationConfig.sequence.find(item=>item.id===observation?.currentBoss);const nextBoss=config.bossRotationConfig.sequence.find(item=>item.id===observation?.nextBoss);
  ...
  document.querySelector('#calendar-current').innerHTML=`
    ...
    <article class="calendar-now-card"><h3>${copy.boss}</h3><span class="calendar-live">${copy.observed}</span><p><strong>${name(boss)}</strong></p><p>${copy.nextUp}: <strong>${name(nextBoss)}</strong></p><span class="calendar-pending">${copy.unknownBoss}</span></article>`;
}
function renderMonth(now=new Date()){
  ...
  document.querySelector('#calendar-days').innerHTML=monthDays(year,month).map(day=>{
    ...
    const boss=key===liveKey&&config.bossRotationConfig.currentObservation?config.bossRotationConfig.sequence.find(item=>item.id===config.bossRotationConfig.currentObservation.currentBoss):null;
    return `<article class="calendar-day${day.inMonth?'':' is-outside'}${key===todayKey?' is-today':''}" data-date="${key}"><div class="calendar-date"><span>${day.day}</span>${key===liveKey?`<span class="calendar-live">${copy.live}</span>`:''}</div><a class="calendar-entry team" href="${localHref(team.event.href)}"><strong>${name(team.event)}</strong><span class="${team.isFinalDay?'is-final':''}">${teamDay}</span></a><span class="calendar-entry island"><strong>${copy.island}</strong><span class="${island.isFinalDay?'is-final':''}">${islandLabel}</span></span>${boss?`<span class="calendar-entry boss"><strong>${copy.boss}</strong><span>${name(boss)}</span></span>`:''}</article>`;
  }).join('');
}
