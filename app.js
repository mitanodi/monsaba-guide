const state={families:[],meta:{},imageByFamily:new Map(),query:'',attribute:'すべて',stage:'all',sort:'default',selectedId:null};
...
async function boot(){
  ...
  state.query=window.__MONSABA_PRIVATE_SEARCH__?.q||''; state.attribute=params.get('attribute')||'すべて'; state.stage=params.get('stage')||'all'; state.sort=params.get('sort')||'default';
  if(!['すべて',...new Set(state.families.map(f=>f.attribute))].includes(state.attribute)) state.attribute='すべて';
  if(!['all','t4','no-t4'].includes(state.stage)) state.stage='all';
...
function bind(){
  ...
  $('#clearFilters').addEventListener('click',()=>{state.query='';state.attribute='すべて';state.stage='all';state.sort='default';$('#search').value='';$('#sort').value='default';syncQuery();renderFilters();renderCards()});
...
function syncQuery(){
  const p=new URLSearchParams(); if(state.attribute!=='すべて')p.set('attribute',state.attribute); if(state.stage!=='all')p.set('stage',state.stage); if(state.sort!=='default')p.set('sort',state.sort);
...
function renderFilters(){
  ...
  let advanced=$('#catalogAdvancedFilters');
  if(!advanced){advanced=document.createElement('div');advanced.id='catalogAdvancedFilters';advanced.className='catalog-advanced-filters';$('#attributeFilters').after(advanced)}
  advanced.innerHTML='<span>進化</span><button type="button" class="filter '+(state.stage==='all'?'is-active':'')+'" data-stage="all">すべて</button><button type="button" class="filter '+(state.stage==='t4'?'is-active':'')+'" data-stage="t4">T4あり</button><button type="button" class="filter '+(state.stage==='no-t4'?'is-active':'')+'" data-stage="no-t4">T4なし</button><div class="active-filter-chips" aria-live="polite">'+(state.attribute!=='すべて'?'<button type="button" data-clear-filter="attribute">'+esc(state.attribute)+'属性 ×</button>':'')+(state.stage!=='all'?'<button type="button" data-clear-filter="stage">'+(state.stage==='t4'?'T4あり':'T4なし')+' ×</button>':'')+'</div>';
  advanced.querySelectorAll('[data-stage]').forEach(b=>b.addEventListener('click',()=>{state.stage=b.dataset.stage;syncQuery();renderFilters();renderCards()}));
  advanced.querySelector('[data-clear-filter="attribute"]')?.addEventListener('click',()=>{state.attribute='すべて';syncQuery();renderFilters();renderCards()});
  advanced.querySelector('[data-clear-filter="stage"]')?.addEventListener('click',()=>{state.stage='all';syncQuery();renderFilters();renderCards()});
}
...
function filteredFamilies(){
  const q=state.query.trim().toLowerCase(); let rows=state.families.filter(f=>(state.attribute==='すべて'||f.attribute===state.attribute)&&(state.stage==='all'||(state.stage==='t4'?f.evolutions.length>=4:f.evolutions.length<4))&&(!q||searchableText(f).includes(q)));
