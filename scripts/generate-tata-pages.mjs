...
<body data-page-type="tata_detail" data-family-id="${esc(family.id)}" data-family-name="${esc(displayLabel)}"><a class="skip-link" href="#main-content">本文へスキップ</a>
  ...
  <main id="main-content">
    <section class="page-hero"><div class="wrap"><nav class="breadcrumbs" aria-label="パンくず"><a href="/">トップ</a><span>›</span><a href="/#tatari">タタ図鑑</a><span>›</span><span>${esc(displayLabel)}</span></nav><div class="family-page-head tata-page-head"><div><span class="attribute">${attr.icon} ${family.attribute}属性</span><h1>${esc(displayLabel)}</h1>${originalName(family.evolutions[0])}<p>${esc(chain)}</p>${roles.length ? `<div class="role-tags tata-role-tags">${roles.map((role) => `<span>${esc(role)}</span>`).join('')}</div>` : ''}</div><div class="tata-hero-actions"><button class="ghost-button tata-favorite-button" type="button" aria-pressed="false">☆ お気に入り</button><button class="ghost-button tata-roster-button" type="button">マイモンサバへ登録</button><a class="button" href="/consult/?flow=detail&amp;family=${encodeURIComponent(family.id)}">このタタを攻略相談所で相談</a><a class="ghost-button" href="/attribute/${attr.slug}/">同じ属性のタタを見る</a></div></div></div></section>
${evolutionIndex}
${quickAnswers}
