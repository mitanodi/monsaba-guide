...
function topCard(family) {
  ...
  return `<article class="card" tabindex="0" role="button" data-family="${esc(family.id)}" aria-label="${esc(family.familyName)}系統を比較表示"><div class="card-image"><img loading="lazy" src="${esc(thumb(first.image))}" width="160" height="160" alt="${esc(first.name)}"></div><div class="card-body"><div class="card-top"><span class="attribute">${icon(family.attribute)} ${esc(family.attribute)}属性</span><span class="source-state">${family.evolutions.length}段階</span></div><h3>${esc(family.familyName)}系</h3><div class="chain">${family.evolutions.map((item) => esc(item.name)).join(' → ')}</div><div class="card-bottom"><span class="skill-chip">${esc(finalSkill?.name || 'スキル')}</span><a class="detail-link" href="/tata/${encodeURIComponent(family.id)}/" aria-label="${esc(family.familyName)}系の個別ページを見る">詳細を見る</a></div></div></article>`;
}
...
function transitionCard(item, includeReason = false) {
  ...
  return `<article class="evolution-card" data-family-id="${esc(item.family.id)}"><div class="evolution-card-head"><img loading="lazy" src="${esc(thumb(item.family.evolutions[0]?.image))}" width="64" height="64" alt="${esc(item.family.familyName)}"><div><span class="attribute">${icon(item.family.attribute)} ${esc(item.family.attribute)}属性</span><h3>${esc(item.family.familyName)}系</h3><p>${esc(item.from.tataName)} → ${esc(item.to.tataName)}</p></div></div><div class="tier-inline">${badge('進化', item.priority)}${badge('総合', overall?.tier)}${zombie ? badge('ゾンビ', zombie) : ''}</div><p class="evolution-headline">${esc(item.headline)}</p>${deltaHtml(item.delta)}${includeReason ? `<p>${esc(item.reason)}</p>` : ''}<a class="detail-link" href="/tata/${encodeURIComponent(item.family.id)}/">詳しく見る</a></article>`;
}
const roadmapGroups = [
...
const roadmap = roadmapGroups.map(([title, items, lead]) => `<article class="priority-tata-card roadmap-card"><h3>${title}</h3><p>${lead}</p>${items.map((item) => { const family = familyById.get(item.familyId); const stageData = skills.byFamily[item.familyId]?.stages || []; const first = stageData[0]; const t3 = stageData.find((stage) => stage.stage === 3); return `<a class="mini-family-row" href="/tata/${encodeURIComponent(item.familyId)}/"><img loading="lazy" src="${esc(thumb(family.evolutions[0]?.image))}" width="46" height="46" alt="${esc(first?.tataName || family.familyName)}"><span><b>${esc(family.familyName)}系</b>${item.familyId === 'gantoru' ? `<small>初期：${esc(first?.tataName)}</small>` : ''}<em>T3：${esc(t3?.tataName)} / ${item.requiredStars}星</em></span></a>`; }).join('')}</article>`).join('');
...
const transitionList = [...transitions].sort((a, b) => (transitionOrder[a.priority] - transitionOrder[b.priority]) || a.family.familyName.localeCompare(b.family.familyName, 'ja')).map((item) => transitionCard(item)).join('');
