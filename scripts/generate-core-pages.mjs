...
function topCard(family) {
  ...
  return `<article class="card" tabindex="0" role="button" data-family="${esc(family.id)}" aria-label="${esc(getFamilyDisplayLabel(family))}を比較表示"><div class="card-image"><img loading="lazy" src="${esc(thumb(first.image))}" width="160" height="160" alt="${esc(first.name)}"></div><div class="card-body"><div class="card-top"><span class="attribute">${icon(family.attribute)} ${esc(family.attribute)}属性</span><span class="source-state">${family.evolutions.length}段階</span></div><h3>${esc(getFamilyDisplayLabel(family))}</h3><div class="chain">${family.evolutions.map((item) => esc(item.name)).join(' → ')}</div><div class="card-bottom"><span class="skill-chip">${esc(finalSkill?.name || 'スキル')}</span><a class="detail-link" href="/tata/${encodeURIComponent(family.id)}/" aria-label="${esc(getFamilyDisplayLabel(family))}の個別ページを見る">詳細を見る</a></div></div></article>`;
}
