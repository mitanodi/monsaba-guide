...
const skillCards = data.tataSkillBalance.map((item) => {
  ...
  const family = familyById.get(item.familyId);
  const mappingNote = item.mappingStatus === 'exact' ? '' : `<p class="zr-mapping-note">公式告知名：${esc(item.officialTataName)}／現DB名：${esc(item.databaseTataName)}（ゲーム内名称を確認待ち）</p>`;
  return `<article class="zr-balance-card" id="balance-${esc(item.familyId)}"><header><div><h3>${esc(item.officialTataName)}</h3><a href="/tata/${encodeURIComponent(item.familyId)}/">${esc(getFamilyDisplayLabel(family))}・T${item.stage}</a></div><span class="zr-direction is-${item.direction}">${direction.symbol} ${direction.label}</span></header>${mappingNote}<div class="zr-skill-list">${item.skills.map((skill) => `<section><h4>${esc(skill.name)}</h4><ul>${skill.changes.map(renderChange).join('')}</ul></section>`).join('')}</div></article>`;
}).join('');
...
<body><a class="skip-link" href="#main-content">本文へスキップ</a>${renderHeader('/updates/2026-08-26/')}
  <main id="main-content">
    <section class="page-hero update-preview-hero"><div class="wrap"><nav class="breadcrumbs" aria-label="パンくず"><a href="/">トップ</a><span>›</span><a href="/updates/">更新履歴</a><span>›</span><span>8/26アップデート実装内容</span></nav><div class="family-page-head"><div><span class="visible-kicker">実装済み・ゲーム内詳細確認中</span><h1>モンサバ 8/26アップデートまとめ</h1><p>パクマ・ゾンビラッシュSeason 1・新T4・バランス調整</p></div></div></div></section>

    <section class="wrap static-section update-article">
      <div class="alert-box update-scheduled-alert"><strong>2026年8月26日 実装済み・詳細確認中</strong><p>公式アップデート告知と公開後の公式投稿をもとに整理しています。Season 1開始、難易度1～4、25ラウンド・750体、ランキング、チップ、作戦時間3時間などは公式告知で確認済みです。パクマの進化名やゲーム内数値などは確認待ちです。</p></div>
      ...
      <section id="new-content"><h2 class="page-h2">新タタ「パクマ」と第4進化</h2><div class="guide-grid"><article class="guide-panel"><h3>水属性の新タタ「パクマ」</h3><p>パクマと進化形態には、ゾンビラッシュで使用される専用スキルが存在します。パクマ自体がゾンビラッシュ専用という意味ではありません。</p><p class="section-note">公式投稿で存在と水属性は確認済みです。T2/T3名称・通常スキル・専用スキル詳細・数値・ゲーム内画像は未確認のため、タタDBにはまだ追加していません。</p></article><article class="guide-panel"><h3>第4進化を2体追加</h3><ul class="plain-list"><li>水属性：スケダコ → <b>ロードパス</b></li><li>岩属性：ボウズヘビ → <b>ナムアミダイジャ</b></li></ul><p class="section-note">名称は公式告知で確認済みです。通常スキル詳細や数値はゲーム内スクリーンショット確認まで推測しません。</p></article></div></section>
      ...
      <section id="other-updates"><h2 class="page-h2">イベント・攻略機能の変更</h2><div class="content-card-grid">
        ...
        <article class="content-card"><h3>不具合修正</h3><p>特定状況下で発生する火山ゾンビ復活の不具合修正が公式告知に掲載されています。</p></article>
      </div></section>

      <section id="chips"><h2 class="page-h2">チップのバランス調整</h2><p>公式アップデート告知に掲載された変更値です。</p><div class="table-wrap"><table class="zombie-table update-balance-table"><thead><tr><th>チップ</th><th>項目</th><th>変更前</th><th>変更後</th><th>区分</th></tr></thead><tbody>${chipRows}</tbody></table></div></section>

      <section id="tata-skills"><h2 class="page-h2">ゾンビラッシュ専用タタスキル調整</h2><div class="summary-box"><strong>通常スキルではありません</strong><p>以下はすべてゾンビラッシュ専用スキルの公式告知値です。矢印は個々の数値変化の方向を示し、「総合的に強い・弱い」を断定するものではありません。</p></div><div class="zr-target-links" aria-label="調整対象35体">${targetLinks}</div><div class="zr-balance-grid">${skillCards}</div></section>

      <section class="source-note"><strong>情報源と確認状況</strong><p>公式ゲーム内の「アップデートのお知らせ」と公式X投稿を最優先ソースとして整理しました。第三者情報から数値を補っていません。特にトラーニー「火焔爆裂」は公式告知の160% → 230%を掲載しています。</p><p>公式告知の「ワンブー」は現DBの「クンブー」、「フクロウグモ」は「フクロクモ」、「ライマー」は「ライメー」に対応する可能性がありますが、既存DB名は変更していません。ゲーム内の系統表示を確認待ちです。</p></section>
    </section>
  </main>
  <footer><div class="wrap footer-inner"><div><strong>モンサバ攻略DB</strong><span>モンスターサバイバル 非公式攻略サイト</span></div><div class="footer-side"><nav class="footer-links" aria-label="サイト情報"><a href="/about/">サイトについて</a><a href="/about-data/">データ方針</a><a href="/updates/">更新履歴</a><a href="/privacy/">プライバシー</a><a href="/friends/">フレンド掲示板</a></nav><p class="footer-contact">お問い合わせ・ご連絡は <a href="https://x.com/odi_monsaba" target="_blank" rel="noopener noreferrer">おぢ（@odi_monsaba）X</a> まで。フォローもよろしくお願いします。</p><div class="footer-meta">8/26アップデート実装内容</div></div></div></footer><script src="/family-display.js"></script><script src="/site.js"></script><script src="/growth.js" defer></script>
</body></html>`;

const outputDirectory = path.join(root, 'updates', '2026-08-26');
fs.mkdirSync(outputDirectory, { recursive: true });
fs.writeFileSync(path.join(outputDirectory, 'index.html'), html);
console.log(`8/26アップデート実装内容ページを生成しました: チップ${data.chipBalance.length}件 / 専用スキル${data.tataSkillBalance.length}体`);
