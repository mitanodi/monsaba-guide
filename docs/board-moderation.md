...
対象は既存Vercel Project `monsaba-guide` の `/board/` です。掲示板は既存Upstash Redis内の `monsaba:board:*` に分離され、`monsaba:friends:*` には触れません。質問・回答は原則保持し、通報レコードだけ180日で整理されます。
...
2. 不審な質問を開き、回答も確認します。URLは自動リンクにならないため、記載URLを直接開かないでください。
3. 通報一覧も続けて確認します。
...
結果の `targetType` が `thread` なら質問、`answer` なら回答です。`targetId` と `reason` を見て、該当する質問詳細を確認します。通報だけで投稿は自動削除されません。

## 削除手順（約1分）

質問を削除すると、その質問に紐づく回答も削除されます。回答だけを削除する場合は `answer` を指定します。必ず対象IDを一度確認してから実行してください。
